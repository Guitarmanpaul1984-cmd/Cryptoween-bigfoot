...
async def chat(update, context):
    ...
    # In groups/supergroups, Bigfoot only replies when his name is used.
    # Examples: "Bigfoot tell me a joke" or "Hey Bigfoot, what's up?"
    if chat_type in ("group", "supergroup"):
        if "bigfoot" not in text.lower():
            return

        # Remove the wake word so the AI gets the actual request.
        cleaned = re.sub(r"\bbigfoot\b[,:!?-]*\s*", "", text, count=1, flags=re.IGNORECASE).strip()
        text = cleaned or "Someone called your name. Give a short funny Bigfoot response."

    ensure_user(update)
    await ai_reply(update, text, 4)
